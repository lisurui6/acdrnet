...
from utils.topology_3d import sample_3d_points
...
class ShapeDeformCardiac3DCircleNet(nn.Module):
    ...
    def __init__(self, num_nodes, enc_dim, voxel_width, voxel_height, voxel_depth, drop, num_lv_slices, num_extra_lv_myo_slices):
        ...
        num_tanh_c0_xy = 2 * num_lv_slices + 2 * num_extra_lv_myo_slices  # c0(x,y) for each slice
        num_sig_lv = 2 * num_lv_slices + num_extra_lv_myo_slices  # (r1, r0/r1) for lv + myo, and r1 for myo
        num_sig_rv = 3 * (num_lv_slices + num_extra_lv_myo_slices)  # theta2/pi, d_c2_c0, theta_c2
        num_sig_dz = num_extra_lv_myo_slices + num_lv_slices - 1  # myo + lv
        # dz = (0, 1) * (1 - c0_z) * depth / (num_extra_lv_myo_slices + num_lv_slices)

        self.shape_end1 = nn.Sequential(nn.Linear(200, num_tanh_c0_xy + 1), nn.Tanh())  # c0_x * (lv + myo), c0_y * (lv + myo) for each slice and c0_z for the first slice
        # bias = torch.from_numpy(np.array([0, 0, 0, 33/128])).float()
        bias = torch.from_numpy(np.array([0] * num_tanh_c0_xy + [-1])).float()

        self.shape_end1[0].weight.data.zero_()
        self.shape_end1[0].bias.data.copy_(bias)

        self.shape_end2 = nn.Sequential(nn.Linear(200, num_sig_lv + num_sig_rv + num_sig_dz), nn.Sigmoid())  # factor 0 (r0/r1), theta2/pi, d_c2_c0, theta_c2 * n_lv + r1 * (lv + myo) + dz (lv, extra, rv)
        # bias = torch.from_numpy(np.array([-1.1838, 0.690, -1.6094, 0, -1.099])).float()
        bias = torch.from_numpy(np.array([0.690] * num_lv_slices + [-1.6094] * (num_lv_slices + num_extra_lv_myo_slices) +  [0] * (num_lv_slices + num_extra_lv_myo_slices) +  [1.9459] * (num_lv_slices + num_extra_lv_myo_slices) + [-1.1838] * (num_lv_slices + num_extra_lv_myo_slices) + [0.5] * num_sig_dz)).float()
    ...
    def forward(self, img, epoch=0):
        ...
        nodes0, tetras0, nodes1, tetras1, nodes2, tetras2, self.tri0, self.tri1, self.tri2 = sample_3d_points(
            par1=out_par1, par2=out_par2, num_lv_slices=self.num_lv_slices, num_extra_lv_myo_slices=self.num_extra_lv_myo_slices,
            voxel_depth=self.voxel_depth, voxel_height=self.voxel_height, voxel_width=self.voxel_width,
            num_points=128, batch_size=img.shape[0], lv_tetras=self.tri0, lv_myo_tetras=self.tri1, rv_tetras=self.tri2,
            epoch=epoch,
        )
