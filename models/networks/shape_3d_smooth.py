...
from utils.topology_3d_smooth import sample_3d_points
...
class ShapeDeformCardiac3DCircleNet(nn.Module):
    ...
    def __init__(self, num_nodes, enc_dim, voxel_width, voxel_height, voxel_depth, drop, num_lv_slices, num_extra_lv_myo_slices):
        ...
        num_tanh_delta = (num_lv_slices + num_extra_lv_myo_slices - 1) * 6 + num_lv_slices - 1
        ...
        bias = torch.from_numpy(np.array([0, 0, -1])).float()
        self.shape_end1[0].weight.data.zero_()
        self.shape_end1[0].bias.data.copy_(bias)

        self.shape_end2 = nn.Sequential(nn.Linear(200, 5), nn.Sigmoid())  # factor 0 (r0/r1), theta2/pi, d_c2_c0, theta_c2, r1, dz (myo + lv - 1)
        # bias = torch.from_numpy(np.array([-1.1838, 0.690, -1.6094, 0, -1.099])).float()
        bias = torch.from_numpy(np.array([0.690, -1.6094, 0, 1.9459, -1.1838])).float()
        ...
        self.shape_end4 = nn.Sequential(nn.Linear(200, num_sig_dz), nn.Sigmoid())
        bias = torch.from_numpy(np.array([0.5] * num_sig_dz)).float()
        self.shape_end4[0].weight.data.zero_()
        self.shape_end4[0].bias.data.copy_(bias)
    ...
    def forward(self, img, epoch=0, vis=False):
        ...
        out_par3 = self.shape_end3(out)  # (B, (num_lv_slices + num_extra_lv_myo_slices - 1) * 6 + num_lv_slices - 1), tanh
        out_par4 = self.shape_end4(out)  # (B, num_extra_lv_myo_slices + num_lv_slices - 1), sig

        out_par1 = torch.cat([out_par1, out_par3], dim=1)
        out_par2 = torch.cat([out_par2, out_par4], dim=1)
        ...
        if self.training:
            return flow, preint_flow, init_mask0, init_mask1, init_mask2, \
                   ...
                   deform_mask[:, 0:1, ...], deform_mask[:, 1:2, ...], deform_mask[:, 2:, ...], out_par1, out_par2
        else:
