...
def batch_sample_lv_myo_points(
    ...
    Args:
        ...
        dx_dz: (B, n_lv + n_myo - 1), dx/dz, (-1, 1) * d_max
        dy_dz: (B, n_lv + n_myo - 1), dx/dz, (-1, 1) * d_max
    ...
    dr1_max = r1 / (num_lv_slices + num_extra_ly_myo_slices - 1)  # (B, 1)
    dr1 = (dr1 - 1) / 2
    ...
    dr0 = (dr0 - 1) / 2
    dr0_max = r0[:, 0].unsqueeze(1) / (num_lv_slices + num_extra_ly_myo_slices - 1)  # (B, 1)
    ...
    return lv_points, lv_myo_points, c0x, c0y, c0z, r1


def batch_sample_rv_points(
        c0x, c0y, c0z, r1, theta_c2, theta2_ratio, d_c2_c0_ratio,
        dz, dtheta2_dz, dtheta_c2_dz, dd_c2_c0_dz,
    ...
    Args:
        ...
        theta2_ratio: (B,)
        d_c2_c0_ratio: (B,)
        dz: (B, n_rv - 1), (0, 1) * (voxel_depth - c0z) / (num_extra_lv_myo_slices + num_lv_slices)
        dtheta2_dz: (B, n_rv - 1), d(theta2)/dz, (-1, 1) * d_theta_max
        dtheta_c2_dz: (B, n_rv - 1), d(theta_c2)/dz, (-1, 1) * d_theta_max
        dd_c2_c0_dz: (B, n_rv - 1), d(d_c2_c0)/dz, (-1, 1) * d_max
        num_points_per_slice:
    ...
    """
    d_max = 0.5
    d_theta_max = math.pi / num_slices
    ...
    dtheta2 = torch.mul(torch.mul(dtheta2_dz, dz), d_theta2_max)  # (B, n_rv - 1)
    ...
    dd_c2_c0 = torch.mul(dd_c2_c0_dz, dz) * d_max  # (B, n_rv - 1)
    ...
    dtheta_c2 = torch.mul(dtheta_c2_dz, dz) * d_theta_max  # (B, n_rv - 1)
...
def sample_3d_points(
    ...
    par1:
        c0_x, c0_y, c0_z,
        dx/dz * (n_lv + n_myo - 1), dy/dz * (n_lv + n_myo - 1), dr1 * (n_lv + n_myo - 1), dr0 * (n_lv - 1),
        dtheta2/dz * (n_lv + n_myo - 1), dtheta_c2/dz * (n_lv + n_myo - 1), dd_c2_c0/dz * (n_lv + n_myo - 1),
    par2:
        factor 0 (r0/r1), theta2/pi, d_c2_c0, theta_c2, r1, dz * (n_lv + n_myo - 1)
    ...
    lv_points, lv_myo_points, c0x, c0y, c0z, r1 = batch_sample_lv_myo_points(
        c0x=par1[:, 0],
        c0y=par1[:, 1],
        c0z=par1[:, 2],
        r1=par2[:, 4],
        r0_r1_ratio=par2[:, 0],
        dz=par2[:, 5:],
        dx_dz=par1[:, 3:(3 + d_slices)],
        dy_dz=par1[:, (3 + d_slices):(3 + d_slices * 2)],
        dr1=par1[:, (3 + d_slices * 2):(3 + d_slices * 3)],
        dr0=par1[:, (3 + d_slices * 3):(2 + d_slices * 3 + num_lv_slices)],
    ...
    rv_points = batch_sample_rv_points(
        ...
        theta_c2=par2[:, 3],
        theta2_ratio=par2[:, 1],
        d_c2_c0_ratio=par2[:, 2],
        dz=par2[:, 5:],
        dtheta2_dz=par1[:, (2 + d_slices * 3 + num_lv_slices):(2 + d_slices * 4 + num_lv_slices)],
        dtheta_c2_dz=par1[:, (2 + d_slices * 4 + num_lv_slices):(2 + d_slices * 5 + num_lv_slices)],
        dd_c2_c0_dz=par1[:, (2 + d_slices * 5 + num_lv_slices):(2 + d_slices * 6 + num_lv_slices)],
        voxel_width=voxel_width,
        voxel_height=voxel_height,
        voxel_depth=voxel_depth,
        num_points_per_slice=num_points,
    ...
    if lv_myo_tetras is None or vis:
