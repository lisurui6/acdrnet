...
def batch_sample_rv_points(
    ...
    x2 = torch.sqrt(1 - (c1z - c1z[:, 0]) * (c1z - c1z[:, 0]) / (dz1 * dz1)) * x2  # (B, n_rv)
...
def batch_sample_lv_myo_points(
    ...
    c1z_end = c0z_0 + (voxel_height - c0z_0) * c1z_end_ratio
    ...
    c0z_end = c0z_end_ratio * (c1z_end - c0z_0) * 0.9 + c0z_0
...
# sample_lv_rv_points(
#     lv_par1=torch.from_numpy(np.array([[0, 0, 0, 1]])).cuda().float(),
#     lv_par2=torch.from_numpy(np.array([[0.1, 0.9, 0.5]])).cuda().float(),
#     rv_par1=torch.from_numpy(np.array([[0, 2/7, 0.75]])).cuda().float(),
#     num_lv_slices=128,
#     voxel_width=64,
#     voxel_depth=64,
#     voxel_height=32,
