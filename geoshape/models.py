...
from geoshape.sample_ellipse import sample_lv_rv_points
...
class LiteShapeDeformNet(torch.nn.Module):
    def __init__(self, voxel_width, voxel_depth, voxel_height, num_lv_slices, num_extra_slices):
        ...
        self.shape_regressor = nn.Sequential(
            nn.Conv3d(
                self.encoder.dims[-2], self.encoder.dims[-2] // 2,
                kernel_size=kernel_size, stride=1, padding=padding
            ),
            nn.BatchNorm3d(self.encoder.dims[-2] // 2),
            nn.ReLU(),
            nn.MaxPool3d((2, 2, 2)),
            nn.Conv3d(self.encoder.dims[-2] // 2, self.encoder.dims[-2] // 4, kernel_size=1, stride=1),
            nn.BatchNorm3d(self.encoder.dims[-2] // 4),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(1024, 400),
            nn.ReLU(),
        ...
        self.flow_decoder = Decoder(
            self.encoder.dims, drop=False, kernel_size=kernel_size, output_act=None,
            output_dim=self.encoder.dims[-4]
        )

        self.flow_conv = torch.nn.Conv3d(self.encoder.dims[-4], 3, kernel_size=3, padding=1)
        # # init flow layer with small weights and bias
        self.flow_conv.weight = torch.nn.Parameter(Normal(0, 1e-5).sample(self.flow_conv.weight.shape))
        self.flow_conv.bias = torch.nn.Parameter(torch.zeros(self.flow_conv.bias.shape))
...
class ShapeDeformNet(torch.nn.Module):
    def __init__(self, voxel_width, voxel_depth, voxel_height, num_lv_slices, num_extra_slices):
        ...
        self.shape_end_lv1 = nn.Sequential(nn.Linear(200, 3), nn.Tanh())  # c0_x, c0_y, c0_z
        bias = torch.from_numpy(np.array([0, 0, -0.5])).float()
        self.shape_end_lv1[0].weight.data.zero_()
        self.shape_end_lv1[0].bias.data.copy_(bias)

        self.shape_end_lv2 = nn.Sequential(nn.Linear(200, 4), nn.Sigmoid())  # r1, r0_r1_ratio, c1z_end_ratio, c0z_end_ratio
        bias = torch.from_numpy(np.array([-1.7, 0.690, 0, 2])).float()
        self.shape_end_lv2[0].weight.data.zero_()
        self.shape_end_lv2[0].bias.data.copy_(bias)
    ...
    def forward(self, img, epoch=0):
        ...
        # )
        [nodes0, tetras0, self.tri0], [nodes1, tetras1, self.tri1], [nodes2, tetras2, self.tri2] = sample_lv_rv_points(
            lv_par1=lv_par1, lv_par2=lv_par2, rv_par1=rv_par_sig,
            num_lv_slices=self.num_lv_slices,
            voxel_width=self.voxel_width, voxel_depth=self.voxel_depth, voxel_height=self.voxel_height,
            num_points=32, batch_size=img.shape[0],
            lv_tetras=self.tri0, lv_myo_tetras=self.tri1, rv_tetras=self.tri2, epoch=epoch,
        )
